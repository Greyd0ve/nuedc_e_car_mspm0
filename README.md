...
| 功能 | 排针 | IO | 外设 | 状态 |
| --- | --- | --- | --- | --- |
| X STEP | H1-5 | PB8 | TIMA0_C0 | 已接入（CC0，尚未实机验证） |
| X DIR | H1-2 | PB15 | GPIO | 已接入 |
| X EN | H13-2 | PB18 | 预留 | 不启用，EN极性待实测 |
| Y STEP | H1-1 | PB5 | TIMA1_C1 | 已接入（CC1，尚未实机验证） |
| Y DIR | H1-3 | PB16 | GPIO | 已接入 |
| Y EN | H13-1 | PB25 | 预留 | 不启用，EN极性待实测 |

注意：
- PA30 已经是 K2，PB0 已经是 K3，因此不能用于 STEP；
- X 轴使用 TIMA0 CC0，Y 轴使用 TIMA1 CC1，两轴独立通道；
- 400 脉冲仍需逻辑分析仪验证；
- D36A EN 有效电平尚未实物确认。
