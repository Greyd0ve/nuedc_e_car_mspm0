...
| `ENC_R_A` | 右电机 M2 A 相 | PA14，A 路中断 |
| `ENC_R_B` | 右电机 M2 B 相 | PA25，B 路方向采样 |

当前第一版使用 A 路 GPIO 双边沿中断计数、B 路读取电平判断方向，并保留 `LEFT_ENCODER_DIR` / `RIGHT_ENCODER_DIR` 方向系数。左右 A 相都在 GPIOA 中断组内，ISR 会分别检查 PA26 和 PA14。
...
3. 编码器 PA26/PA27/PA14/PA25 是否和实际接口接线一致。
